...
Fire up an IRB session and enter the following command: `require 'open-uri'`. This command loads a method called `open` from the Ruby Standard Library (which is part of Ruby but isn't loaded up by default because not every single Ruby script needs to open Internet pages). The `open` method takes one argument: a String containing a URL.

Create a variable called `url` and store within it a String containing the URL we discovered that has the data we want and pass it to the `open` method:
...
I prefer working in small steps and peeling one layer off at a time while I am exploring.

#### Your Task

Now that we've seen how to retrieve information from a URL using IRB, let's plug it in to a real Ruby script.

**Fork** this repo to your own account and clone **your fork** to your own computer. Open a command line window, `cd` to the folder that you just downloaded, and open it up in Sublime as well.

I have started you off with three files: `address_to_coords.rb`, `coords_to_weather.rb`, and `address_to_weather.rb`.

#### Address to Coords

First, read through  `address_to_coords.rb`. There's one new thing: I have used a method `URI.encode` which we also get for free from the `open-uri` library. If you pass it a string, it automatically replaces spaces and other illegal URL characters for us.

Your job is to get an address from the user and tell them the latitude and longitude of it. I have included some hints in the file.

When I run your program from the command line with `ruby address_to_coords.rb`, if I type in `5807 S Woodlawn Ave`, I should see

    The latitude of 5807 S Woodlawn Ave is 41.7896234 and the longitude is -87.5964137.

#### Coords to Weather

Next, in `coords_to_weather.rb`, you will do something similar; but instead of using Google's Geocoding API, you will use The Forecast API. We will exchange a latitude/longitude pair for weather information. Forecast is an amazing API that gives you minute-by-minute meteorological information.
...
It's pretty amazingly detailed data, but all we need for now is the current temperature and the minutely and hourly summaries.

But first, can we customize the example to get data relevant to us? Plug in some coordinates that you fetched in `address_to_coords.rb` and try it out.

Your job is to get a latitude and a longitude from the user and tell them the current temperature at that location, as well as the outlook for the next hour and day.
